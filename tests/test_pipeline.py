...
class TestAnalyzeFile(unittest.TestCase):
    ...
    def test_returns_list_of_saved_paths(self):
        with tempfile.TemporaryDirectory() as vault:
            result = pipeline.analyze_file(SAMPLE_C_PATH, vault, use_llm=False)
    ...
    def test_saved_notes_are_markdown_files(self):
        with tempfile.TemporaryDirectory() as vault:
            for path in pipeline.analyze_file(SAMPLE_C_PATH, vault, use_llm=False):
                self.assertTrue(path.endswith(".md"), f"Expected .md, got: {path}")

    def test_max_functions_limits_notes(self):
        with tempfile.TemporaryDirectory() as vault:
            result = pipeline.analyze_file(SAMPLE_C_PATH, vault, use_llm=False, max_functions=1)
        self.assertEqual(len(result), 1)

    def test_skip_existing_skips_already_written_notes(self):
        with tempfile.TemporaryDirectory() as vault:
            first_run = pipeline.analyze_file(SAMPLE_C_PATH, vault, use_llm=False)
            second_run = pipeline.analyze_file(SAMPLE_C_PATH, vault, use_llm=False, skip_existing=True)
        # second run should produce no new notes (all already exist)
        self.assertEqual(len(second_run), 0)

    def test_no_skip_rewrites_existing_notes(self):
        with tempfile.TemporaryDirectory() as vault:
            first = pipeline.analyze_file(SAMPLE_C_PATH, vault, use_llm=False)
            second = pipeline.analyze_file(SAMPLE_C_PATH, vault, use_llm=False, skip_existing=False)
        # Both runs should produce the same number of notes
        self.assertEqual(len(first), len(second))

    def test_note_content_includes_prompt_when_no_llm(self):
        with tempfile.TemporaryDirectory() as vault:
            paths = pipeline.analyze_file(SAMPLE_C_PATH, vault, use_llm=False)
            with open(paths[0]) as f:
                content = f.read()
        # Without LLM the note should include the raw prompt section
        self.assertIn("Prompt", content)

    def test_llm_result_written_to_note(self):
        import json
        ...
        with tempfile.TemporaryDirectory() as vault:
            with patch('pipeline.query_ollama', return_value=(mock_payload, 38.5)):
                paths = pipeline.analyze_file(SAMPLE_C_PATH, vault, use_llm=True, max_functions=1)
            with open(paths[0]) as f:
                content = f.read()
    ...
    def test_vault_subdirectory_created_per_file(self):
        with tempfile.TemporaryDirectory() as vault:
            pipeline.analyze_file(SAMPLE_C_PATH, vault, use_llm=False)
