...
#### Summary
Creates a new llama_sampler instance with the same llg context as the input sampler.
#### Details
Initializes the new sampler with the same vocabulary and then copies the state from the input sampler, including the grammar and tokenizer.
#### Performance
Time complexity: O(n), where n is the size of the grammar and tokenizer data.
#### Rationale
Ensures that the new sampler has the same context as the input sampler, while also allowing for efficient copying of the state.
#### Where Used
* llama_sampler_llg_init
* llama_sampler_llg_free
#### Tags
